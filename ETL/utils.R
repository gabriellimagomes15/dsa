...
## Função para fazer web scrapping na página vagas de emprego do glassdoor
scrapGlassDoor <- function(urls = '', totalPages = 10){
  ...
  ## Loop para identificar aumtomaticamente o país 
  ...
  for(i in 1:dim(urlDF)[1]){
    ...
    tryCatch({
      ## FOR PARA LER AS PÁGINAS COM TODOS AS VAGAS...TO READ THE PAGES WITH ALL JOB POST. TotalPages+1 is because the first page doesn't work with this url
      for( pag in 1:totalPages){
        ...
        tryCatch({    
          ...
          for(href in links){
            ...
            data   <- data.frame(id, positJob, company, city_state, date, descrip, dateColect, country = urlDF$country[i], url)
...
#### >>>> COMPANIES REVIEWS (Avaliação das Empresas) ####
    ...
    for( page in 1:totalPages){
      ...
      tryCatch({
        ...
        for(review in nodes){
          ...
          #O ID FICA EM UM ATRIBUTO DO ELEMENTO HTML, ENTÃO, ESTÁ PEGANDO TODOS OS ATRIBUTOS DO ELEMENTO ONTE ESTÁ O ID E CONVERTENDO PARA VETOR CHAR
          id    <- review %>% html_attrs() %>% unlist(.) %>% paste(.,collapse = ' ') %>% gsub('[[:alpha:]]|\\s|[[:punct:]]','', .) #html_attr('id') 
          cat('\n\t id = ', id)
          
          #O RATING FICA EM UM ATRIBUTO DO ELEMENTO HTML, ENTÃO, ESTÁ PEGANDO TODOS OS ATRIBUTOS DO ELEMENTO ONTE ESTÁ O ID E CONVERTENDO PARA VETOR CHAR
          ...
          dataFrame <- data.frame(id, rating, company, dateColec, date, title, status, location, recommend, outlook, ceo, pros, cons, adviceManag)
...
### FUNÇÃO PARA REALIZAR WEB SCRAPPING DE AVALIAÇÕES SOBRE EMPRESAS. 
 ## PARAMETROS: 
  # nodes = NODES QUE CONTEM AS AVAILAÇOES. OS NODES SÃO PASSADOS COMO PARAMETRO APÓS JÁ REALIZAR O WEB SCRAPPING NA CLASSE PRINCIPAL(MAIN)
  # tagDate = TAG(CLASSE/ID) PARA RECUPERAR A DATA DA AVALIAÇÃO
  # tagTitle = TAG(CLASSE/ID) PARA RECUPERAR O TÍTULO DA AVALIAÇÃO
  # tagStatus = TAG(CLASSE/ID) PARA RECUPERAR O STATUS DO AVALIAÇÃO (CARGO, EMPREGADO, EX-EMPREGADO ETC)
  # tagLocation = TAG(CLASSE/ID) PARA RECUPERAR A LOCALIZAÇÃO DE ONDE FOI FEITA A AVALIAÇÃO
  # tagRecommend = TAG(CLASSE/ID) PARA RECUPERAR SE O AVALIADOR RECOMENDA OU NÃO A EMPRESA
  # tagOutlook = TAG(CLASSE/ID) PARA RECUPERAR SE PERSPECTIVA É BOA, INTERMEDIARIA OU RUIM (glassdoor)
  # tagCeo = TAG(CLASSE/ID) PARA RECUPERAR SE APROVA O CEO DA EMPRESA
  # tagPros = TAG(CLASSE/ID) PARA RECUPERAR AVALIAÇÃO POSITIVA (PRÓS DA EMPRESA)
  # tagCons = = TAG(CLASSE/ID) PARA RECUPERAR AVALIAÇÃO NEGATIVA (CONTRAS DA EMPRESA)
  # tagAdvice = TAG(CLASSE/ID) PARA RECUPERAR MENSAGEM PARA CONSELHOS
  # company = NOME DA EMPRESA QUE SERÁ RECUPERADO NA CLASSE PRINCIPAL 
  # ids = IDS RECUPERADOS NA CLASSE PRINCIPAL E SERÃO INSERIDOS NO FINAL NO DATAFRAME. (Os ids são passados como parametro, porque são 
    # inseridos na página de diferentes formas, inclusive passando como atributo da tag, assim, sendo necessário outra metodologia para recupera-los)
  # ratings = RATING RECUPERADOS NA CLASSE PRINCIPAL E SERÃO INSERIDOS NO FINAL NO DATAFRAME. (Os rating são passados como parametro, porque são 
    # inseridos na página de diferentes formas, inclusive passando como atributo da tag, assim, sendo necessário outra metodologia para recupera-los)
...
clearCity <- function(data, column = ''){
  ...
  cityClean <- sapply(data, function(x){
    x <- gsub("--", 'NI',x)
    x <- gsub("â€“", '',x) ## CLEAR ESPECIAL CHAR IN LINUX
...
#### >>>> REVIEWS COMPANIES (avaliações das empresas) ####

convertDate <- function(data){
  return <- data.frame(t( apply(data,1,function(x){
            if(grepl('dias|dias',x['date']) ){
              days <- as.numeric( gsub("^.*([0-9]+).*$", "\\1", x['date']) )
              x['dateClear'] <- as.character( as.Date(x['dateCollect']) - days)
            }else if(grepl('meses|mes',x['date'] )){
              days <- as.numeric( gsub("^.*([0-9]+).*$", "\\1", x['date']) ) * 30
              x['dateClear'] <- as.character( as.Date(x['dateCollect']) - days)
            }else if(grepl('anos|anos',x['date'] )){
              days <- as.numeric( gsub("^.*([0-9]+).*$", "\\1", x['date']) ) * 365
              x['dateClear'] <- as.character( as.Date(x['dateCollect']) - days)
            }else{
              x['dateClear'] <- x['dateCollect']
            }
            x
          })), stringsAsFactors = F)
  return(return)
...
# >>>> END (FIM) REVIEWS COMPANIES (avaliações das empresas) ####
...
## Obs: Para eliminar a duplicidade de registro, serÃ¡ utilizada a primeira coluna do parametro 'column';
cleanText <- function(data,column = '', stopWords = "",stemming = F,specialChar = F,delDup = F){
  print('Clean Data')
  if(column[1] == ''){
    warning('ATENÃ‡ÃƒO!!!!\n A coluna (atributo) selecionada para limpeza Ã©: ', colnames(data)[1])
  ...
  if(delDup){
    ...
    warning('\n ===> ATENÃ‡ÃƒO!!!\n SerÃ¡ verificado a duplicidade de registros na coluna : ', toupper(column[1]),'\n')
